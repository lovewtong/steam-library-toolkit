...
def get_owned_games(api_key: str, steam_id: str, include_non_inventory: bool = True, *, access_token=None, stats=None):
    ...
    resp = get_response(url, params=params, timeout=15, stats=stats)
        ...
        try:
            ...
            def native_source(name, payload, scope):
                ...
                return SourceResult(name, records, status="ok" if state == "complete" else "failed",
                                    ...
                                    attempts=payload.get("attempts") if type(payload.get("attempts")) is int else 0)
    ...
    if use_api and not any(r.source == "web_api" for r in results):
        ...
        try:
            ...
            records = get_owned_games(config.get("api_key", ""), account,
                                      include_non_inventory, stats=api_stats)
            results.append(SourceResult("web_api", records, steam_id=account, scope="api_visible", attempts=api_stats.get("attempts", 0)))
...
def main():
    ...
    parser.add_argument("--diagnose", action="store_true", help="仅输出环境与依赖诊断，不读取凭据、不联网")
    ...
    try:
        ...
        with collection_lock(args.output, paths):
        ...
        from steam_runs import save_failed_run
        import re
        match = re.match(r"([A-Z][A-Z0-9_]{1,63})[：:]", message)
        try:
            diagnostic = save_failed_run(args.output, report, match.group(1) if match else "COLLECTION_FAILED")
            print(f"失败诊断：{diagnostic}", flush=True)
        except OSError:
            print("失败诊断无法写入；原运行指针保持不变。", flush=True)
