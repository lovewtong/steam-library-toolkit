# 报告落地：采集契约与 picker 修复（待发布）

基线为 `a28e4de`。此文档是发布说明草稿，不表示远端已经发布，也不采信研究报告中“远端当前版本”的时效性结论；实际发布前需重新核对远端。
...
新增 Windows/Linux/macOS 离线 CI 定义，不访问真实 Steam 账号。`requirements-tested.txt` 固定本次 Windows 已验的直接依赖；不冒充跨平台完整传递依赖锁。

源码模式扫描不扫描 Git 历史、不替代全面 secrets scanner。发布前应单独确认历史与 diff 不包含账号数据、凭据和本机产物；尤其旧收藏脚本仍有硬编码账号等历史问题。本次不会执行 push、创建公开发布或导入 Steam 收藏。
...
- 独立补全后回归为 62 项 Python、17 项 Node 测试通过。对真实库中的 500、550、730 三个应用补全成功，第二次全部命中缓存；输出仍为 388 条成员、377 game，原库及成员/类型/时长证据保持不变。本轮只验证了三个应用的商店请求，未执行全库补全。
