...
2026-09-12 已通过 Windows 本机登录态严格模式实测：客户端 388 条、API 358 条，补回 30 条，默认分类选择 377 个 game。361 条有明确时长，27 条仍未知；这只验证本次账号的采集链路，不能证明任何账号的绝对全集。两个分类入口已统一类型选择与运行校验；picker 仍读取平铺分类输出，使用自定义采集文件时应先通过 `classify_steam_games.py -i 文件名` 更新它。详见 [README.md](README.md)。
