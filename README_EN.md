...
Standalone enrichment accepts `--workers` from 1 to 4 (default 2), with shared request pacing and cached responses. Category IDs fix false controller-support negatives caused by localized descriptions. Missing fields preserve prior values, and audits include before/after field coverage. See [metadata enrichment](METADATA_ENRICHMENT.md) for usage, cache v4 migration and measured results.
