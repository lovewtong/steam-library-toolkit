...
async function call(service, method, token, params, fetcher = fetch) {
  let response;
  try {
    response = await fetcher(`https://api.steampowered.com/${service}/${method}/v1/?` +
      new URLSearchParams({access_token: token, ...params}), {redirect: 'error', signal: AbortSignal.timeout(35000)});
  } catch (_) { throw new SourceError('NETWORK_UNAVAILABLE'); }
  if (response.status !== 200 || !['1', null].includes(response.headers.get('x-eresult')))
    throw new SourceError('STEAM_REQUEST_FAILED');
  let body;
...
async function clientLibrary(token, account, machine, fetcher) {
  const getSessions = async () => {
    const body = await call('IClientCommService', 'GetAllClientLogonInfo', token, {}, fetcher);
  ...
  const read = async () => {
    const body = await call('IClientCommService', 'GetClientAppList', token,
      {client_instanceid: session.client_instanceid, fields: 'games', include_client_info: 'true', language: 'schinese'}, fetcher);
...
async function ownedGames(token, account, expanded, fetcher) {
  const body = await call('IPlayerService', 'GetOwnedGames', token, {steamid: account, include_appinfo: 'true',
    include_played_free_games: String(expanded), include_free_sub: String(expanded), skip_unvetted_apps: String(!expanded)}, fetcher);
...
async function collectWebSources(token, request, account, fetcher) {
  ...
  try {
    const safe = async fn => { try { return await fn(); } catch (e) { return failed(e); } };
    const [client, api] = await Promise.all([
      safe(() => clientLibrary(token, account, request.machine, fetcher)),
      request.use_api === false ? Promise.resolve(null) : safe(() => ownedGames(token, account, request.expanded !== false, fetcher)),
    ]);
